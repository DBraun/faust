...
- The JAX backend generates several functions:
  1. `__init__(self, *args, **kwargs) -> None:`
...
In `__init__`, we are allowed to assign attributes to `self`.
For example, a commonly autogenerated property is a constant related to the sample-rate:
```python
self._fConst0 = np.minimum(np.float32(1.92e+05), np.maximum(np.float32(1.0), (self._fSampleRate)))
```

Other attributes/parameters are set indirectly because `__init__` may call `self.add_hslider(...)`,
which is implemented by an architecture file, which almost certainly calls `self.param(...)`

`__init__` is also where signals for rdtables are created. They are constants, so they shouldn't be in the "state"
...
void JAXCodeContainer::produceClass()
{
    ...
    *fOut << R"(
    ...
    if (gGlobal->gFloatSize == 2) {
        ...
        *fOut << R"(
		faust_float: Dtype = jnp.float64,
		faust_int: Dtype = jnp.int64,)";
    } else {
        *fOut << R"(
    ...
    *fOut << "# fmt: off";
    ...
    // __init__ method - handles only immutable instance attributes and constants
    {
        JAXInstVisitor* jaxVisitor = static_cast<JAXInstVisitor*>(gGlobal->gJAXVisitor);
        // Use RAII context scope for __init__ method
