...
```
This is also where signals for rdtables are created.
...
`tick` is the closest thing to a "compute" method compared to other backends. It computes one new sample,
return a tuple of the new state and the new audio, which could have multiple channels.
