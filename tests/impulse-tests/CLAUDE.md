...
- Or use NumPy arrays during initialization (mutable), then convert to JAX later
...
1. **Focus on specific failing tests** - Instead of running the full test suite, test individual DSP files:
   ...
   $FAUST -lang nnx dsp/table2.dsp -a archs/impulsennx.py -double > ir/nnx/double/nnx_table2.py
...
- Grep for specific patterns: `grep -B5 -A5 "pattern" file` # such as "setup"/"tick"/"_initialize_carry"
