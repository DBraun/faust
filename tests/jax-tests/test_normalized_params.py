...
if __name__ == "__main__":
    print("="*60)
    print("Testing Normalized Params for RL Integration")
    print("="*60)

    all_passed = True

    for test_func in [
        test_normalized_params_single,
        test_normalized_params_vmap,
        test_get_default_params_normalized,
        test_params_vs_normalized_params,
    ]:
        try:
            if not test_func():
                all_passed = False
    ...
    if all_passed:
        ...
        print()
        print("RL Integration Pattern:")
        print()
        print("  # 1. Policy outputs normalized params [0, 1]")
        print("  policy_output = beta_distribution(features)  # Shape: [batch, num_params]")
        print()
        print("  # 2. Convert to dict format")
        print("  normalized_batch = {'fHslider0': policy_output[:, 0], ...}")
        print()
        print("  # 3. Simple vmap with normalized_params")
        print("  @jax.vmap")
        print("  def render(norm_params, inputs, rng):")
        print("      return model(inputs, normalized_params=norm_params, rngs=rng)")
        print()
        print("  audio = render(normalized_batch, inputs_batch, rngs_batch)")
        print()
        print("No manual unnormalization needed!")
    else:
