...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow4 = (jnp.float32(2.0) * ((fSlow2 + fSlow3) + jnp.float32(-1.0))) 
		fSlow5 = ((jnp.sqrt(fSlow2) * jnp.sin(fSlow0)) / jnp.maximum(jnp.float32(0.001), params["fHslider2"])) 
		fSlow6 = ((fSlow2 + jnp.float32(-1.0)) * fSlow1) 
		fSlow7 = (fSlow2 + fSlow6) 
		fSlow8 = (fSlow7 + (jnp.float32(1.0) - fSlow5)) 
		fSlow9 = ((fSlow5 + fSlow7) + jnp.float32(1.0)) 
		fSlow10 = (jnp.float32(1.0) / fSlow9) 
		fSlow11 = (fSlow2 + (jnp.float32(1.0) - (fSlow6 + fSlow5))) 
		fSlow12 = (jnp.float32(2.0) * (fSlow2 + (jnp.float32(-1.0) - fSlow3))) 
		fSlow13 = ((fSlow2 + fSlow5) + (jnp.float32(1.0) - fSlow6)) 
		fSlow14 = (fSlow2 / fSlow9) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow10 * ((fSlow8 * state["fRec0"][2]) - (fSlow4 * state["fRec0"][1]))))) 
