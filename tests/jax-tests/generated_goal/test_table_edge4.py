...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("test_table_edge4") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		iTemp0 = (state["iRec1"] % jnp.int32(10)) 
		state["iRec2"] = ((jnp.int32(2) * iRec2_temp) + jnp.int32(1)) 
		state["itbl0"] = state["itbl0"].at[iTemp0].set(state["iRec2"]) 
		_result0 = state["itbl0"][((iTemp0 + 2) % 10)] 
		iTemp1 = (state["iRec1"] % jnp.int32(7)) 
		state["fRec4"] = ((jnp.float32(1.5) * fRec4_temp) + jnp.float32(0.5)) 
		state["ftbl1"] = state["ftbl1"].at[iTemp1].set(state["fRec4"]) 
		_result1 = state["ftbl1"][((iTemp1 + 2) % 7)] 
