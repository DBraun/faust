...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		state["iRec1"] = ((jnp.int32(2) * iRec1_temp) + jnp.int32(1)) 
		state["iRec2"] = (iRec2_temp + jnp.int32(1)) 
		iTemp0 = (state["iRec2"] % jnp.int32(10)) 
		state["itbl0"] = state["itbl0"].at[iTemp0].set(state["iRec1"]) 
		_result0 = (state["itbl0"][((iTemp0 + 2) % 10)]) 
		state["fRec4"] = ((jnp.float32(1.5) * fRec4_temp) + jnp.float32(0.5)) 
		iTemp1 = (state["iRec2"] % jnp.int32(7)) 
