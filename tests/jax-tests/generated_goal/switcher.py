...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("switcher") 
		self.add_hslider("fHslider0", ui_path, "source 0 <-> source 1", 0.0, 0.0, 1.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		_result0 = ((fSlow0 * inputs[2]) + (fSlow1 * inputs[0])) 
		_result1 = ((fSlow0 * inputs[3]) + (fSlow1 * inputs[1])) 
