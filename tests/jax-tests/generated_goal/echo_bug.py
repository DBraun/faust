...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("echo_bug") 
		self.add_hslider("fHslider0", ui_path, "EchoWarp", 0.0, -1.0, 1.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fRec1"] = np.zeros((2048,), dtype=np.float32)
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fTemp0 = inputs[0] 
		state["fRec1"] = state["fRec1"].at[(state["IOTA0"] & 2047).astype(jnp.int32)].set(((jnp.float32(0.995) * state["fRec1"][((state["IOTA0"] - 1) & 2047).astype(jnp.int32)]) + (jnp.float32(0.005) * fTemp0))) 
		fRec3 = params["fHslider0"] 
		fRec2 = (jnp.float32(1e+02) * (fRec3 + jnp.float32(1.0))) 
		iTemp1 = jnp.int32(fRec2) 
		fTemp2 = jnp.floor(fRec2) 
		fRec0 = ((state["fRec1"][((state["IOTA0"] - jnp.minimum(1025, jnp.maximum(0, iTemp1))) & 2047).astype(jnp.int32)] * (fTemp2 + (jnp.float32(1.0) - fRec2))) + ((fRec2 - fTemp2) * state["fRec1"][((state["IOTA0"] - jnp.minimum(1025, jnp.maximum(0, (iTemp1 + 1)))) & 2047).astype(jnp.int32)])) 
		_result0 = (fTemp0 + fRec0) 
