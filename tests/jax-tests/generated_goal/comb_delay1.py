...
class mydsp(nn.Module):
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fVec0"] = np.zeros((11,), dtype=np.float32)
	...
	def setup(self):
		...
		self._fConst1 = (np.float32(4.4e+02) / self._fConst0) 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		state["fRec1"] = (self._fConst1 + (fRec1_temp - jnp.floor((self._fConst1 + fRec1_temp)))) 
		state["fVec0"] = state["fVec0"].at[0].set(self._ftbl0mydspSIG0[jnp.maximum(0, jnp.minimum(jnp.int32((jnp.float32(65536.0) * state["fRec1"])), 65535))]) 
		state["fRec2"] = (self._fConst2 + (fRec2_temp - jnp.floor((self._fConst2 + fRec2_temp)))) 
		state["fVec1"] = state["fVec1"].at[(state["IOTA0"] & 127).astype(jnp.int32)].set(state["fVec0"][jnp.int32(jnp.maximum(jnp.float32(0.0), jnp.minimum(jnp.float32(1e+01), (jnp.float32(5.0) * (self._ftbl0mydspSIG0[jnp.maximum(0, jnp.minimum(jnp.int32((jnp.float32(65536.0) * state["fRec2"])), 65535))] + jnp.float32(1.0))))))]) 
		_result0 = state["fVec1"][((state["IOTA0"] - 100) & 127).astype(jnp.int32)] 
