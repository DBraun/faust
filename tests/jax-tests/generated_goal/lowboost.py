...
class mydsp(nnx.Module):
	...
	def __init__(
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("low-freq shelving boost") 
		self.add_hslider("fHslider0", ui_path, "freq", 1e+03, 2e+01, 2e+04, unnorm_funcs, "linear") 
		self.add_hslider("fHslider1", ui_path, "gain", 0.0, -2e+01, 2e+01, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow1 = (jnp.float32(1.0) / ((fSlow0 * (fSlow0 + jnp.float32(1.4142135))) + jnp.float32(1.0))) 
		fSlow2 = jnp.power(jnp.float32(1e+01), (jnp.float32(0.05) * params["fHslider1"])) 
		fSlow3 = jnp.sqrt((jnp.float32(2.0) * fSlow2)) 
		fSlow4 = (fSlow2 * fSlow0) 
		fSlow5 = ((fSlow0 * (fSlow3 + fSlow4)) + jnp.float32(1.0)) 
		fSlow6 = ((fSlow0 * (fSlow0 + jnp.float32(-1.4142135))) + jnp.float32(1.0)) 
		fSlow7 = jnp.power(fSlow0, jnp.float32(2.0)) 
		fSlow8 = (jnp.float32(2.0) * (fSlow7 + jnp.float32(-1.0))) 
		fSlow9 = (jnp.float32(2.0) * ((fSlow2 * fSlow7) + jnp.float32(-1.0))) 
		fSlow10 = (jnp.float32(1.0) - (fSlow0 * (fSlow3 - fSlow4))) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow1 * ((fSlow6 * state["fRec0"][2]) + (fSlow8 * state["fRec0"][1]))))) 
		_result0 = (fSlow1 * (((fSlow5 * state["fRec0"][0]) + (fSlow9 * state["fRec0"][1])) + (fSlow10 * state["fRec0"][2]))) 
