...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow2 = (jnp.float32(2.0) * (fSlow1 + jnp.float32(-1.0))) 
		fSlow3 = ((fSlow0 * (fSlow0 + jnp.float32(-1.4142135))) + jnp.float32(1.0)) 
		fSlow4 = (jnp.float32(1.0) / ((fSlow0 * (fSlow0 + jnp.float32(1.4142135))) + jnp.float32(1.0))) 
		fSlow5 = jnp.power(jnp.float32(1e+01), (jnp.float32(0.05) * params["fHslider1"])) 
		fSlow6 = (fSlow5 * fSlow0) 
		fSlow7 = jnp.sqrt((jnp.float32(2.0) * fSlow5)) 
		fSlow8 = (jnp.float32(1.0) - (fSlow0 * (fSlow7 - fSlow6))) 
		fSlow9 = (jnp.float32(2.0) * ((fSlow5 * fSlow1) + jnp.float32(-1.0))) 
		fSlow10 = ((fSlow0 * (fSlow7 + fSlow6)) + jnp.float32(1.0)) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow4 * ((fSlow3 * state["fRec0"][2]) + (fSlow2 * state["fRec0"][1]))))) 
		_result0 = (fSlow4 * (((fSlow10 * state["fRec0"][0]) + (fSlow9 * state["fRec0"][1])) + (fSlow8 * state["fRec0"][2]))) 
