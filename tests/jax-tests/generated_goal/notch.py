...
class mydsp(nnx.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow3 = (jnp.float32(1.0) - fSlow1) 
		fSlow4 = (jnp.float32(2.0) * jnp.cos(fSlow0)) 
		fTemp0 = (fSlow4 * state["fRec0"][1]) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow2 * ((fSlow3 * state["fRec0"][2]) - fTemp0)))) 
		_result0 = (fSlow2 * (state["fRec0"][2] + (state["fRec0"][0] - fTemp0))) 
