...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow4 = (jnp.float32(1.0) / (fSlow2 + jnp.float32(1.0))) 
		fTemp0 = (fSlow1 * state["fRec0"][1]) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow4 * ((fSlow3 * state["fRec0"][2]) - fTemp0)))) 
		_result0 = (state["fRec0"][2] + (fSlow4 * ((fSlow3 * state["fRec0"][0]) - fTemp0))) 
