...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = params["fButton0"] 
		fSlow1 = params["fButton1"] 
		fSlow2 = (jnp.float32(0.1) * params["fCheckbox0"]) 
		...
		state["fVec0"] = fSlow0 
		state["fVec1"] = fSlow1 
		iTemp0 = ((fSlow1 - fVec1_temp) > jnp.float32(0.0)).astype(jnp.int32) 
		...
		fTemp1 = (fSlow2 * (((jnp.float32(0.049922034) * state["fRec0"][0]) + (jnp.float32(0.0506127) * state["fRec0"][2])) - ((jnp.float32(0.095993534) * state["fRec0"][1]) + (jnp.float32(0.004408786) * state["fRec0"][3])))) 
		_result0 = (((((fSlow0 - fVec0_temp) > jnp.float32(0.0)).astype(jnp.int32) + iTemp0)) + fTemp1) 
		state["fVec2"] = fSlow3 
		_result1 = (fTemp1 + ((iTemp0 + ((fSlow3 - fVec2_temp) > jnp.float32(0.0)).astype(jnp.int32)))) 
