...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("Stereo Reverb Tester") 
		ui_path.append("Input Config") 
		self.add_button("fCheckbox0", ui_path, "Pink Noise", unnorm_funcs) 
		ui_path.pop()
		ui_path.append("Impulse Selection") 
		self.add_button("fButton1", ui_path, "Left", unnorm_funcs) 
		self.add_button("fButton0", ui_path, "Center", unnorm_funcs) 
		self.add_button("fButton2", ui_path, "Right", unnorm_funcs) 
		ui_path.pop()
		ui_path.pop()
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fRec0"] = np.zeros((4,), dtype=np.float32)
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = (jnp.float32(0.1) * params["fCheckbox0"]) 
		fSlow1 = params["fButton0"] 
		fSlow2 = params["fButton1"] 
		fSlow3 = params["fButton2"] 
		...
		state["fRec0"] = state["fRec0"].at[0].set((((jnp.float32(0.5221894) * state["fRec0"][3]) + ((jnp.float32(4.656613e-10) * (state["iRec1"])) + (jnp.float32(2.494956) * state["fRec0"][1]))) - (jnp.float32(2.0172658) * state["fRec0"][2]))) 
		fTemp0 = (fSlow0 * (((jnp.float32(0.049922034) * state["fRec0"][0]) + (jnp.float32(0.0506127) * state["fRec0"][2])) - ((jnp.float32(0.095993534) * state["fRec0"][1]) + (jnp.float32(0.004408786) * state["fRec0"][3])))) 
		state["fVec0"] = fSlow1 
		iTemp1 = ((fSlow1 - fVec0_temp) > jnp.float32(0.0)).astype(jnp.int32) 
		state["fVec1"] = fSlow2 
		_result0 = (((((fSlow2 - fVec1_temp) > jnp.float32(0.0)).astype(jnp.int32) + iTemp1)) + fTemp0) 
		state["fVec2"] = fSlow3 
		_result1 = (fTemp0 + ((iTemp1 + ((fSlow3 - fVec2_temp) > jnp.float32(0.0)).astype(jnp.int32)))) 
