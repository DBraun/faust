...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("delays") 
		self.add_nentry("fEntry0", ui_path, "d1", 2e+02, 0.0, 1e+03, 1.0, unnorm_funcs, "linear") 
		self.add_nentry("fEntry1", ui_path, "d2", 2e+02, 0.0, 2e+03, 1.0, unnorm_funcs, "linear") 
		self.add_nentry("fEntry2", ui_path, "d3", 4e+02, 2e+02, 3e+03, 1.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fVec0"] = np.zeros((1024,), dtype=np.float32)
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		iSlow0 = jnp.int32(params["fEntry0"]) 
		iSlow1 = jnp.int32(params["fEntry1"]) 
		iSlow2 = jnp.int32(params["fEntry2"]) 
		fRec0_temp = state["fRec0"] 
		fRec1_temp = state["fRec1"] 
		fRec2_temp = state["fRec2"] 
		state["fVec0"] = state["fVec0"].at[(state["IOTA0"] & 1023).astype(jnp.int32)].set(inputs[0]) 
		state["fRec0"] = ((jnp.float32(0.999) * fRec0_temp) + (jnp.float32(0.001) * state["fVec0"][((state["IOTA0"] - iSlow0) & 1023).astype(jnp.int32)])) 
		_result0 = state["fRec0"] 
		state["fVec1"] = state["fVec1"].at[(state["IOTA0"] & 2047).astype(jnp.int32)].set(inputs[1]) 
		state["fRec1"] = ((jnp.float32(0.999) * fRec1_temp) + (jnp.float32(0.001) * state["fVec1"][((state["IOTA0"] - iSlow1) & 2047).astype(jnp.int32)])) 
		_result1 = state["fRec1"] 
		state["fVec2"] = state["fVec2"].at[(state["IOTA0"] & 4095).astype(jnp.int32)].set(inputs[2]) 
		state["fRec2"] = ((jnp.float32(0.999) * fRec2_temp) + (jnp.float32(0.001) * state["fVec2"][((state["IOTA0"] - iSlow2) & 4095).astype(jnp.int32)])) 
		_result2 = state["fRec2"] 
		state["IOTA0"] = (state["IOTA0"] + jnp.int32(1)) 
		return state, jnp.stack([_result0,_result1,_result2]) 
