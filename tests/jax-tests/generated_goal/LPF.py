...
class mydsp(nnx.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow2 = (jnp.float32(0.5) * (jnp.sin(fSlow0) / jnp.maximum(jnp.float32(0.001), params["fHslider1"]))) 
		fSlow3 = (fSlow2 + jnp.float32(1.0)) 
		fSlow4 = ((jnp.float32(1.0) - fSlow1) / fSlow3) 
		fSlow5 = (jnp.float32(1.0) / fSlow3) 
		fSlow6 = (jnp.float32(1.0) - fSlow2) 
		fSlow7 = (jnp.float32(2.0) * fSlow1) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow5 * ((fSlow6 * state["fRec0"][2]) - (fSlow7 * state["fRec0"][1]))))) 
		_result0 = (fSlow4 * ((state["fRec0"][1] + (jnp.float32(0.5) * state["fRec0"][0])) + (jnp.float32(0.5) * state["fRec0"][2]))) 
