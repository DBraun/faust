...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow5 = (fSlow3 + jnp.float32(1.0)) 
		fSlow6 = (jnp.float32(1.0) / fSlow5) 
		fSlow7 = ((jnp.float32(1.0) - fSlow1) / fSlow5) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow6 * ((fSlow4 * state["fRec0"][2]) - (fSlow2 * state["fRec0"][1]))))) 
		_result0 = (fSlow7 * ((state["fRec0"][1] + (jnp.float32(0.5) * state["fRec0"][0])) + (jnp.float32(0.5) * state["fRec0"][2]))) 
