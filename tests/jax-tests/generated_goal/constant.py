...
class mydsp(nn.Module):
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fVec1"] = np.zeros((3,), dtype=np.float32)
		state["fVec2"] = np.zeros((5,), dtype=np.float32)
	...
	def setup(self):
		...
		self._iConst2 = (self._fConst0 == np.float32(0.0)).astype(jnp.int32) 
		
		self._iConst3 = (jnp.int32(np.floor(self._fConst0)) % np.int32(2)) 
		
		self._iConst4 = (jnp.int32(np.floor((np.float32(0.5) * self._fConst0))) % np.int32(2)) 
		
		self._iConst5 = (jnp.int32(np.floor((np.float32(0.25) * self._fConst0))) % np.int32(2)) 
		
		self._iConst6 = (self._iConst3 + (np.int32(2) * self._iConst4)) 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		state["fVec1"] = state["fVec1"].at[0].set(fTemp1) 
		state["fVec2"] = state["fVec2"].at[0].set((fTemp1 + state["fVec1"][2])) 
		_result0 = (self._fConst1 * ((jnp.where((self._iConst2 != 0), fTemp0, jnp.where((self._iConst3 != 0), fTemp0, jnp.float32(0.0))) + jnp.where((self._iConst4 != 0), state["fVec1"][self._iConst3], jnp.float32(0.0))) + jnp.where((self._iConst5 != 0), state["fVec2"][self._iConst6], jnp.float32(0.0)))) 
		state["fVec1"] = jnp.roll(state["fVec1"], 1) 
		state["fVec2"] = jnp.roll(state["fVec2"], 1) 
