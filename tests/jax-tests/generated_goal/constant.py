...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self._iConst1 = (np.int32(np.floor((np.float32(0.25) * self._fConst0))) % np.int32(2)) 
		
		self._iConst2 = (np.int32(np.floor((np.float32(0.5) * self._fConst0))) % np.int32(2)) 
		
		self._iConst3 = (np.int32(np.floor(self._fConst0)) % np.int32(2)) 
		
		self._iConst4 = (self._iConst3 + (np.int32(2) * self._iConst2)) 
		
		self._iConst5 = (self._fConst0 == np.float32(0.0)).astype(jnp.int32) 
		
		self._fConst6 = (np.float32(1.0) / self._fConst0) 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		_result0 = (self._fConst6 * ((jnp.where((self._iConst5 != 0), fTemp0, jnp.where((self._iConst3 != 0), fTemp0, jnp.float32(0.0))) + jnp.where((self._iConst2 != 0), state["fVec1"][self._iConst3], jnp.float32(0.0))) + jnp.where((self._iConst1 != 0), state["fVec2"][self._iConst4], jnp.float32(0.0)))) 
