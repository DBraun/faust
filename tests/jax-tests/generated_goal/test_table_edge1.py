...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		ftbl0mydspSIG0 = np.zeros((5,), dtype=np.float32)
		# Initialize waveform data
		self._fmydspSIG0Wave0 = np.array([5.5,-2.3,8.1,0.0,-4.7], dtype=np.float32)
		...
		for i1 in range(0, 5):
			...
			fmydspSIG0Wave0_idx = ((np.int32(1) + fmydspSIG0Wave0_idx) % np.int32(5)) 
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("test_table_edge1") 
		ui_path.pop()
		
		self._unnorm_funcs = unnorm_funcs
		# Initialize other constants
		self._fConst0 = self._ftbl0mydspSIG0[0] 
