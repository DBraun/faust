...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self.add_hslider("fHslider1", ui_path, "freq", 1e+03, 2e+01, 2.4e+04, unnorm_funcs, "linear") 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		_result0 = (state["fRec0"] * self._ftbl0mydspSIG0[jnp.maximum(0, jnp.minimum(jnp.int32((jnp.float32(65536.0) * state["fRec2"])), 65535))]) 
