...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self._imydspSIG0Wave0 = np.array([10,20,30,40,50,60], dtype=np.int32)
		...
		for i1 in range(0, 6):
			...
			imydspSIG0Wave0_idx = ((np.int32(1) + imydspSIG0Wave0_idx) % np.int32(6)) 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		_result0 = (inputs[1] * jnp.sin((self._itbl0mydspSIG0[jnp.int32((jnp.float32(4.0) * jnp.abs(inputs[0])))]))) 
