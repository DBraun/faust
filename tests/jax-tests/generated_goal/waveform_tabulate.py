...
class mydsp(nn.Module):
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		for i1 in range(0, 4):
			...
			state["iRec0"] = (iRec0_temp + np.int32(1)) 
			state["itbl1mydspSIG0"][i1] = self._itbl0mydspSIG0SIG0[np.maximum(0, np.minimum(jnp.int32((np.float32(1.3333334) * (np.maximum(0, np.minimum(iRec0_temp, 3))))), 3))] 
	...
	def setup(self):
		...
		iRec0 = np.int32(0)
		fillmydspSIG0SIG0(dsp, np.int32(4), itbl0mydspSIG0SIG0) 
		instanceInitmydspSIG0SIG0(dsp, sample_rate) 
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("waveform_tabulate") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		fTemp0 = (jnp.float32(0.75) * inputs[0]) 
		iTemp1 = jnp.int32(fTemp0) 
		iTemp2 = state["itbl1mydspSIG0"][jnp.maximum(0, jnp.minimum(iTemp1, 3))] 
		_result0 = ((iTemp2) + ((fTemp0 - (iTemp1)) * ((state["itbl1mydspSIG0"][jnp.maximum(0, jnp.minimum((iTemp1 + 1), 3))] - iTemp2)))) 
