...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self._fmydspWave0 = np.array([0.1,0.123,0.142567,0.15263739,0.568453,0.1,0.123,0.142567,0.15263739,0.568453], dtype=np.float32)
		# Convert static tables and waveform data to JAX arrays
		# Initialize UI parameters
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("waveform1") 
		ui_path.pop()
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fmydspWave0"] = np.zeros((10,), dtype=np.float32)
		return state
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		state["fmydspWave0_idx"] = ((jnp.int32(1) + state["fmydspWave0_idx"]) % jnp.int32(10)) 
