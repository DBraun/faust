...
class mydsp(nn.Module):
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["ftbl3"] = np.zeros((4,), dtype=np.float32)
		...
		# WARNING: Unknown initialization pattern for ftbl3
		...
		# WARNING: Unknown initialization pattern for itbl2
		# Table will remain initialized to zeros
		return state
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("test_table_brittle") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		_result0 = state["itbl0"][(state["iRec1"] % 8)] 
		_result1 = state["ftbl1"][(state["iRec1"] % 6)] 
		_result2 = state["itbl2"][(state["iRec1"] % 5)] 
		_result3 = state["ftbl3"][(state["iRec1"] % 4)] 
