...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		fRec0 = np.zeros((4,), dtype=np.float32)
		# iRec1

		iRec1 = np.int32(0)
		for i1 in range(0, 65536):
			iRec1_temp = iRec1 
			iRec1 = ((np.int32(1103515245) * iRec1_temp) + np.int32(12345)) 
			fRec0[0] = (((np.float32(0.5221894) * fRec0[3]) + ((np.float32(4.656613e-10) * (iRec1)) + (np.float32(2.494956) * fRec0[1]))) - (np.float32(2.0172658) * fRec0[2])) 
			ftbl0mydspSIG0[i1] = (((np.float32(0.049922034) * fRec0[0]) + (np.float32(0.0506127) * fRec0[2])) - ((np.float32(0.095993534) * fRec0[1]) + (np.float32(0.004408786) * fRec0[3]))) 
			fRec0 = np.roll(fRec0, 1) 
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("table") 
		self.add_hslider("fHslider0", ui_path, "freq", 4.4e+02, 5e+01, 2e+03, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = (self._fConst0 * params["fHslider0"]) 
		fRec2_temp = state["fRec2"] 
		state["fRec2"] = (fSlow0 + (fRec2_temp - jnp.floor((fSlow0 + fRec2_temp)))) 
		fTemp0 = self._ftbl0mydspSIG0[jnp.maximum(0, jnp.minimum(jnp.int32((jnp.float32(65536.0) * state["fRec2"])), 65535))] 
		_result0 = fTemp0 
		_result1 = fTemp0 
