...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("NoiseMD") 
		self.add_vslider("fVslider0", ui_path, "Volume", 0.0697, 0.0, 1.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = (jnp.float32(4.656613e-10) * params["fVslider0"]) 
		iRec0_temp = state["iRec0"] 
		state["iRec0"] = ((jnp.int32(1103515245) * iRec0_temp) + jnp.int32(12345)) 
		_result0 = (fSlow0 * (state["iRec0"])) 
