...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow3 = jnp.sin(fSlow0) 
		fSlow4 = (jnp.float32(0.5) * (fSlow3 / fSlow2)) 
		fSlow5 = (jnp.float32(1.0) - fSlow4) 
		fSlow6 = (fSlow4 + jnp.float32(1.0)) 
		fSlow7 = (jnp.float32(1.0) / fSlow6) 
		fSlow8 = (jnp.float32(0.5) * (fSlow3 / (fSlow2 * fSlow6))) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow7 * ((fSlow5 * state["fRec0"][2]) - (fSlow1 * state["fRec0"][1]))))) 
		_result0 = (fSlow8 * (state["fRec0"][0] - state["fRec0"][2])) 
