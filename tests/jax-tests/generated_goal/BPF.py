...
class mydsp(nnx.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow3 = (jnp.float32(0.5) * (fSlow1 / fSlow2)) 
		fSlow4 = (fSlow3 + jnp.float32(1.0)) 
		fSlow5 = (jnp.float32(0.5) * (fSlow1 / (fSlow2 * fSlow4))) 
		fSlow6 = (jnp.float32(1.0) / fSlow4) 
		fSlow7 = (jnp.float32(1.0) - fSlow3) 
		fSlow8 = (jnp.float32(2.0) * jnp.cos(fSlow0)) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow6 * ((fSlow7 * state["fRec0"][2]) - (fSlow8 * state["fRec0"][1]))))) 
		_result0 = (fSlow5 * (state["fRec0"][0] - state["fRec0"][2])) 
