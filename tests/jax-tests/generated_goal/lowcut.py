...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self.add_hslider("fHslider0", ui_path, "attenuation", 0.0, -96.0, 1e+01, unnorm_funcs, "linear") 
		self.add_hslider("fHslider1", ui_path, "freq", 1e+02, 2e+01, 5e+03, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow1 = jnp.tan((self._fConst0 * params["fHslider1"])) 
		fSlow2 = jnp.power(fSlow1, jnp.float32(2.0)) 
		fSlow3 = (jnp.float32(2.0) * ((fSlow2 * fSlow0) + jnp.float32(-1.0))) 
		fSlow4 = (fSlow1 * fSlow0) 
		fSlow5 = jnp.sqrt((jnp.float32(2.0) * fSlow0)) 
		fSlow6 = (jnp.float32(1.0) - (fSlow1 * (fSlow5 - fSlow4))) 
		fSlow7 = (jnp.float32(1.0) / ((fSlow1 * (fSlow5 + fSlow4)) + jnp.float32(1.0))) 
		fSlow8 = ((fSlow1 * (fSlow1 + jnp.float32(-1.4142135))) + jnp.float32(1.0)) 
		fSlow9 = (jnp.float32(2.0) * (fSlow2 + jnp.float32(-1.0))) 
		fSlow10 = ((fSlow1 * (fSlow1 + jnp.float32(1.4142135))) + jnp.float32(1.0)) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow7 * ((fSlow6 * state["fRec0"][2]) + (fSlow3 * state["fRec0"][1]))))) 
		_result0 = (fSlow7 * (((fSlow10 * state["fRec0"][0]) + (fSlow9 * state["fRec0"][1])) + (fSlow8 * state["fRec0"][2]))) 
