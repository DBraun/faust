...
class mydsp(nnx.Module):
	...
	def __init__(
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("low-freq shelving cut") 
		self.add_hslider("fHslider1", ui_path, "attenuation", 0.0, -96.0, 1e+01, unnorm_funcs, "linear") 
		self.add_hslider("fHslider0", ui_path, "freq", 1e+02, 2e+01, 5e+03, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = jnp.tan((self._fConst0 * params["fHslider0"])) 
		fSlow1 = jnp.power(jnp.float32(1e+01), -((jnp.float32(0.05) * params["fHslider1"]))) 
		fSlow2 = jnp.sqrt((jnp.float32(2.0) * fSlow1)) 
		fSlow3 = (fSlow0 * fSlow1) 
		fSlow4 = (jnp.float32(1.0) / ((fSlow0 * (fSlow2 + fSlow3)) + jnp.float32(1.0))) 
		fSlow5 = ((fSlow0 * (fSlow0 + jnp.float32(1.4142135))) + jnp.float32(1.0)) 
		fSlow6 = (jnp.float32(1.0) - (fSlow0 * (fSlow2 - fSlow3))) 
		fSlow7 = jnp.power(fSlow0, jnp.float32(2.0)) 
		fSlow8 = (jnp.float32(2.0) * ((fSlow7 * fSlow1) + jnp.float32(-1.0))) 
		fSlow9 = (jnp.float32(2.0) * (fSlow7 + jnp.float32(-1.0))) 
		fSlow10 = ((fSlow0 * (fSlow0 + jnp.float32(-1.4142135))) + jnp.float32(1.0)) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow4 * ((fSlow6 * state["fRec0"][2]) + (fSlow8 * state["fRec0"][1]))))) 
		_result0 = (fSlow4 * (((fSlow5 * state["fRec0"][0]) + (fSlow9 * state["fRec0"][1])) + (fSlow10 * state["fRec0"][2]))) 
