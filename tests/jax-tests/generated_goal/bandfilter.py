...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow7 = (jnp.float32(1.0) - (fSlow0 * (fSlow6 - fSlow0))) 
		fSlow8 = ((fSlow0 * (fSlow0 + fSlow6)) + jnp.float32(1.0)) 
		fTemp0 = (fSlow1 * state["fRec0"][1]) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow5 * ((fSlow4 * state["fRec0"][2]) + fTemp0)))) 
		_result0 = (fSlow5 * ((fTemp0 + (fSlow8 * state["fRec0"][0])) + (fSlow7 * state["fRec0"][2]))) 
