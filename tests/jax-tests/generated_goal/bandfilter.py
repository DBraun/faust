...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("Bandfilter") 
		self.add_nentry("fEntry1", ui_path, "Q factor", 5e+01, 0.1, 1e+02, 0.1, unnorm_funcs, "linear") 
		self.add_nentry("fEntry0", ui_path, "freq", 1e+03, 2e+01, 2e+04, 1.0, unnorm_funcs, "linear") 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow5 = ((fSlow0 * (fSlow0 - fSlow2)) + jnp.float32(1.0)) 
		fSlow6 = (jnp.float32(1.0) / fSlow1) 
		fSlow7 = ((fSlow0 * (fSlow0 + fSlow6)) + jnp.float32(1.0)) 
		fSlow8 = (jnp.float32(1.0) - (fSlow0 * (fSlow6 - fSlow0))) 
		fTemp0 = (fSlow4 * state["fRec0"][1]) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow3 * ((fSlow5 * state["fRec0"][2]) + fTemp0)))) 
		_result0 = (fSlow3 * ((fTemp0 + (fSlow7 * state["fRec0"][0])) + (fSlow8 * state["fRec0"][2]))) 
