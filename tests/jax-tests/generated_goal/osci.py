...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self.add_hslider("fHslider1", ui_path, "freq", 564.0, 2e+01, 2.4e+04, unnorm_funcs, "linear") 
		self.add_hslider("fHslider0", ui_path, "volume", 0.0, -96.0, 0.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow1 = (self._fConst0 * params["fHslider1"]) 
		fRec0_temp = state["fRec0"] 
		fRec2_temp = state["fRec2"] 
		state["fRec0"] = (fSlow0 + (jnp.float32(0.999) * fRec0_temp)) 
		state["fRec2"] = (fSlow1 + (fRec2_temp - jnp.floor((fSlow1 + fRec2_temp)))) 
		...
		_result0 = (state["fRec0"] * (fTemp2 + ((fTemp0 - jnp.floor(fTemp0)) * (self._ftbl0mydspSIG0[jnp.maximum(0, jnp.minimum((iTemp1 + 1), 65536))] - fTemp2)))) 
