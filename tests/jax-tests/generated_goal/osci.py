...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("Oscillator") 
		self.add_hslider("fHslider0", ui_path, "freq", 564.0, 2e+01, 2.4e+04, unnorm_funcs, "linear") 
		self.add_hslider("fHslider1", ui_path, "volume", 0.0, -96.0, 0.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow1 = (jnp.float32(0.001) * jnp.power(jnp.float32(1e+01), (jnp.float32(0.05) * params["fHslider1"]))) 
		...
		state["fRec2"] = (fSlow1 + (jnp.float32(0.999) * fRec2_temp)) 
		_result0 = (state["fRec2"] * (fTemp2 + ((fTemp0 - jnp.floor(fTemp0)) * (self._ftbl0mydspSIG0[jnp.maximum(0, jnp.minimum((iTemp1 + 1), 65536))] - fTemp2)))) 
