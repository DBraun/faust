...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("highShelf") 
		self.add_hslider("fHslider1", ui_path, "Freq", 1e+03, 1e+02, 1e+04, unnorm_funcs, "linear") 
		self.add_hslider("fHslider0", ui_path, "Gain", 0.0, -1e+01, 1e+01, unnorm_funcs, "linear") 
		self.add_hslider("fHslider2", ui_path, "Q", 1.0, 0.01, 1e+02, unnorm_funcs, "linear") 
		ui_path.pop()
		
		self._unnorm_funcs = unnorm_funcs
		# Initialize other constants
		self._fConst0 = (np.float32(6.2831855) / np.minimum(np.float32(1.92e+05), np.maximum(np.float32(1.0), (self.sample_rate)))) 
		
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		
		fSlow0 = jnp.power(jnp.float32(1e+01), (jnp.float32(0.025) * params["fHslider0"])) 
		fSlow1 = (self._fConst0 * jnp.maximum(jnp.float32(0.0), params["fHslider1"])) 
		fSlow2 = ((jnp.sqrt(fSlow0) * jnp.sin(fSlow1)) / jnp.maximum(jnp.float32(0.001), params["fHslider2"])) 
		fSlow3 = jnp.cos(fSlow1) 
		fSlow4 = ((fSlow0 + jnp.float32(-1.0)) * fSlow3) 
		fSlow5 = ((fSlow0 + fSlow2) + (jnp.float32(1.0) - fSlow4)) 
		fSlow6 = (fSlow0 / fSlow5) 
		fSlow7 = (fSlow0 + fSlow4) 
		fSlow8 = ((fSlow7 + fSlow2) + jnp.float32(1.0)) 
		fSlow9 = (jnp.float32(1.0) / fSlow5) 
		fSlow10 = (fSlow0 + (jnp.float32(1.0) - (fSlow4 + fSlow2))) 
		fSlow11 = ((fSlow0 + jnp.float32(1.0)) * fSlow3) 
		fSlow12 = (jnp.float32(2.0) * (fSlow0 + (jnp.float32(-1.0) - fSlow11))) 
		fSlow13 = (jnp.float32(2.0) * ((fSlow0 + fSlow11) + jnp.float32(-1.0))) 
		fSlow14 = (fSlow7 + (jnp.float32(1.0) - fSlow2)) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow9 * ((fSlow10 * state["fRec0"][2]) + (fSlow12 * state["fRec0"][1]))))) 
		_result0 = (fSlow6 * (((fSlow8 * state["fRec0"][0]) - (fSlow13 * state["fRec0"][1])) + (fSlow14 * state["fRec0"][2]))) 
