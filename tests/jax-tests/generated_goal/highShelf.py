...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self.add_hslider("fHslider0", ui_path, "Freq", 1e+03, 1e+02, 1e+04, unnorm_funcs, "linear") 
		self.add_hslider("fHslider1", ui_path, "Gain", 0.0, -1e+01, 1e+01, unnorm_funcs, "linear") 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = (self._fConst0 * jnp.maximum(jnp.float32(0.0), params["fHslider0"])) 
		fSlow1 = jnp.cos(fSlow0) 
		fSlow2 = jnp.power(jnp.float32(1e+01), (jnp.float32(0.025) * params["fHslider1"])) 
		fSlow3 = ((fSlow2 + jnp.float32(1.0)) * fSlow1) 
		fSlow4 = (jnp.float32(2.0) * (fSlow2 + (jnp.float32(-1.0) - fSlow3))) 
		fSlow5 = ((jnp.sqrt(fSlow2) * jnp.sin(fSlow0)) / jnp.maximum(jnp.float32(0.001), params["fHslider2"])) 
		fSlow6 = ((fSlow2 + jnp.float32(-1.0)) * fSlow1) 
		fSlow7 = (fSlow2 + (jnp.float32(1.0) - (fSlow6 + fSlow5))) 
		fSlow8 = ((fSlow2 + fSlow5) + (jnp.float32(1.0) - fSlow6)) 
		fSlow9 = (jnp.float32(1.0) / fSlow8) 
		fSlow10 = (fSlow2 + fSlow6) 
		fSlow11 = (fSlow10 + (jnp.float32(1.0) - fSlow5)) 
		fSlow12 = (jnp.float32(2.0) * ((fSlow2 + fSlow3) + jnp.float32(-1.0))) 
		fSlow13 = ((fSlow10 + fSlow5) + jnp.float32(1.0)) 
		fSlow14 = (fSlow2 / fSlow8) 
		state["fRec0"] = state["fRec0"].at[0].set((inputs[0] - (fSlow9 * ((fSlow7 * state["fRec0"][2]) + (fSlow4 * state["fRec0"][1]))))) 
		_result0 = (fSlow14 * (((fSlow13 * state["fRec0"][0]) - (fSlow12 * state["fRec0"][1])) + (fSlow11 * state["fRec0"][2]))) 
