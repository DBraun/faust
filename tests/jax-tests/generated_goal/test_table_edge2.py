...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self._imydspSIG0Wave0 = np.array([100,200,300,400,500,600,700,800,900,1000], dtype=np.int32)
		# Convert static tables and waveform data to JAX arrays
		# Initialize UI parameters
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("test_table_edge2") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		iTemp0 = (state["iRec0"] % jnp.int32(10)) 
		state["itbl0"] = state["itbl0"].at[iTemp0].set(jnp.int32(0)) 
		_result0 = state["itbl0"][iTemp0] 
