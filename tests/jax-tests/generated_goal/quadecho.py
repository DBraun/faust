...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("stereo echo") 
		ui_path.append("echo 1000") 
		self.add_hslider("fHslider1", ui_path, "feedback", 1e+02, 0.0, 1e+02, unnorm_funcs, "linear") 
		self.add_hslider("fHslider0", ui_path, "millisecond", 108.7, 0.0, 1e+03, unnorm_funcs, "linear") 
		ui_path.pop()
		ui_path.pop()
		
		self._unnorm_funcs = unnorm_funcs
		# Initialize other constants
		self._fConst0 = (np.float32(0.001) * np.minimum(np.float32(1.92e+05), np.maximum(np.float32(1.0), (self.sample_rate)))) 
		
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		state = {}
		
		# Initialize array delays
		state["fRec0"] = np.zeros((131072,), dtype=np.float32)
		state["fRec1"] = np.zeros((131072,), dtype=np.float32)
		state["fRec2"] = np.zeros((131072,), dtype=np.float32)
		state["fRec3"] = np.zeros((131072,), dtype=np.float32)
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		iSlow0 = (((jnp.int32((self._fConst0 * params["fHslider0"])) + jnp.int32(-1)) & jnp.int32(65535)).astype(jnp.int32) + jnp.int32(1)) 
		fSlow1 = (jnp.float32(0.01) * params["fHslider1"]) 
		state["fRec0"] = state["fRec0"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[0] + (fSlow1 * state["fRec0"][((state["IOTA0"] - iSlow0) & 131071).astype(jnp.int32)]))) 
		_result0 = state["fRec0"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
		state["fRec1"] = state["fRec1"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[1] + (fSlow1 * state["fRec1"][((state["IOTA0"] - iSlow0) & 131071).astype(jnp.int32)]))) 
		_result1 = state["fRec1"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
		state["fRec2"] = state["fRec2"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[2] + (fSlow1 * state["fRec2"][((state["IOTA0"] - iSlow0) & 131071).astype(jnp.int32)]))) 
		_result2 = state["fRec2"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
		state["fRec3"] = state["fRec3"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[3] + (fSlow1 * state["fRec3"][((state["IOTA0"] - iSlow0) & 131071).astype(jnp.int32)]))) 
		_result3 = state["fRec3"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
