...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self.add_hslider("fHslider0", ui_path, "feedback", 1e+02, 0.0, 1e+02, unnorm_funcs, "linear") 
		self.add_hslider("fHslider1", ui_path, "millisecond", 108.7, 0.0, 1e+03, unnorm_funcs, "linear") 
		ui_path.pop()
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		iSlow1 = (((jnp.int32((self._fConst0 * params["fHslider1"])) + jnp.int32(-1)) & jnp.int32(65535)).astype(jnp.int32) + jnp.int32(1)) 
		state["fRec0"] = state["fRec0"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[0] + (fSlow0 * state["fRec0"][((state["IOTA0"] - iSlow1) & 131071).astype(jnp.int32)]))) 
		_result0 = state["fRec0"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
		state["fRec1"] = state["fRec1"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[1] + (fSlow0 * state["fRec1"][((state["IOTA0"] - iSlow1) & 131071).astype(jnp.int32)]))) 
		_result1 = state["fRec1"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
		state["fRec2"] = state["fRec2"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[2] + (fSlow0 * state["fRec2"][((state["IOTA0"] - iSlow1) & 131071).astype(jnp.int32)]))) 
		_result2 = state["fRec2"][(state["IOTA0"] & 131071).astype(jnp.int32)] 
		state["fRec3"] = state["fRec3"].at[(state["IOTA0"] & 131071).astype(jnp.int32)].set((inputs[3] + (fSlow0 * state["fRec3"][((state["IOTA0"] - iSlow1) & 131071).astype(jnp.int32)]))) 
