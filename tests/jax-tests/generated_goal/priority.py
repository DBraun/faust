...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self._iConst1 = np.int32(self._fConst0) 
		
		self._iConst2 = np.int32((np.float32(0.5) * self._fConst0)) 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		state["iRec3"] = ((iVec1_temp + iRec3_temp) % self._iConst1) 
		iTemp0 = (state["iRec3"] <= iRec3_temp).astype(jnp.int32) 
		state["iVec2"] = iTemp0 
		state["iRec2"] = (iRec2_temp + (iTemp0 > iVec2_temp).astype(jnp.int32)) 
		state["iRec5"] = ((iVec1_temp + iRec5_temp) % self._iConst2) 
		iTemp1 = (state["iRec5"] <= iRec5_temp).astype(jnp.int32) 
		state["iVec3"] = iTemp1 
		state["iRec4"] = (iRec4_temp + (iTemp1 > iVec3_temp).astype(jnp.int32)) 
		fTemp2 = (fRec1_temp + (self._fConst3 * ((jnp.int32(200) * (((state["iRec4"] % jnp.int32(2)) + (jnp.int32(2) * (state["iRec2"] % jnp.int32(2)))) + jnp.int32(1)))))) 
