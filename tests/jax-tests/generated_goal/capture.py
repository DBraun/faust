...
class mydsp(nnx.Module):
	...
	def __init__(
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("Audio Capture") 
		self.add_button("fButton0", ui_path, "Capture", unnorm_funcs) 
		self.add_hslider("fHslider0", ui_path, "level (db)", 0.0, -96.0, 4.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		iSlow2 = jnp.int32(fSlow0) 
		fSlow3 = (jnp.float32(0.001) * jnp.power(jnp.float32(1e+01), (jnp.float32(0.05) * params["fHslider0"]))) 
		iVec1_temp = state["iVec1"] 
		iRec1_temp = state["iRec1"] 
		fRec0_temp = state["fRec0"] 
		fRec2_temp = state["fRec2"] 
		state["fVec0"] = state["fVec0"].at[(state["IOTA0"] & 524287).astype(jnp.int32)].set(((fSlow1 * fRec0_temp) + (fSlow0 * inputs[0]))) 
		state["iVec1"] = iSlow2 
		state["iRec1"] = ((iSlow2 + iRec1_temp) * ((iSlow2 - iVec1_temp) <= jnp.int32(0)).astype(jnp.int32)) 
		state["fRec0"] = state["fVec0"][((state["IOTA0"] - ((state["iRec1"] + -1) & 524287).astype(jnp.int32)) & 524287).astype(jnp.int32)] 
		state["fRec2"] = (fSlow3 + (jnp.float32(0.999) * fRec2_temp)) 
		_result0 = (state["fRec0"] * state["fRec2"]) 
