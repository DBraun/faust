...
class mydsp(nn.Module):
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		state["fVec0"] = state["fVec0"].at[(state["IOTA0"] & 524287).astype(jnp.int32)].set(((fSlow2 * fRec1_temp) + (fSlow1 * inputs[0]))) 
		state["iVec1"] = iSlow3 
		state["iRec2"] = ((iSlow3 + iRec2_temp) * ((iSlow3 - iVec1_temp) <= jnp.int32(0)).astype(jnp.int32)) 
		state["fRec1"] = state["fVec0"][((state["IOTA0"] - ((state["iRec2"] + -1) & 524287).astype(jnp.int32)) & 524287).astype(jnp.int32)] 
		_result0 = (state["fRec1"] * state["fRec0"]) 
