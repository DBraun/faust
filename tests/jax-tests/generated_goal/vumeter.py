...
class mydsp(nnx.Module):
	...
	def __init__(
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("vumeter") 
		self.add_hbargraph("fHbargraph0", ui_path, "hbargraph0", -7e+01, 5.0, unnorm_funcs) 
		self.add_hbargraph("fHbargraph1", ui_path, "hbargraph1", -7e+01, 5.0, unnorm_funcs) 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fTemp0 = inputs[0] 
		state["fRec0"] = jnp.maximum((fRec0_temp - self._fConst0), jnp.abs(fTemp0)) 
		fHbargraph0 = (jnp.float32(2e+01) * jnp.log10(jnp.maximum(jnp.float32(0.00031622776), state["fRec0"])))
		# self.sow("intermediates", "fHbargraph0", fHbargraph0) 
		_result0 = fTemp0 
		fTemp1 = inputs[1] 
		state["fRec1"] = jnp.maximum((fRec1_temp - self._fConst0), jnp.abs(fTemp1)) 
		fHbargraph1 = (jnp.float32(2e+01) * jnp.log10(jnp.maximum(jnp.float32(0.00031622776), state["fRec1"])))
