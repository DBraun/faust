...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		self._fmydspWave0 = np.array([0.1,0.56,1.3,8.9], dtype=np.float32)
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["fmydspWave0"] = np.zeros((4,), dtype=np.float32)
		return state
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fTemp1 = jnp.sqrt((jnp.float32(3.0) * fTemp0)) 
		_result0 = fTemp1 
		_result1 = jnp.sqrt(fTemp1) 
		fTemp2 = (jnp.float32(4.0) * fTemp0) 
		_result2 = jnp.power(fTemp2, jnp.float32(0.125)) 
		_result3 = jnp.power(fTemp2, jnp.float32(0.0625)) 
		fTemp3 = jnp.power((jnp.float32(5.0) * fTemp0), jnp.float32(2.0)) 
		_result4 = fTemp3 
		_result5 = fTemp3 
		iTemp4 = jnp.int32((jnp.float32(6.0) * fTemp0)) 
		_result6 = (jnp.int32(jnp.power((iTemp4), (jnp.int32(2))))) 
		_result7 = jnp.power((iTemp4), jnp.float32(2.0)) 
		fTemp5 = (jnp.float32(7.0) * fTemp0) 
		_result8 = jnp.power(fTemp5, jnp.float32(2.0001)) 
		_result9 = jnp.power(fTemp5, jnp.float32(2.0)) 
		_result10 = jnp.float32(1.0) 
		_result11 = (jnp.float32(8.0) * fTemp0) 
		_result12 = jnp.float32(1.0) 
		_result13 = (jnp.float32(9.0) * fTemp0) 
		state["fmydspWave0_idx"] = ((jnp.int32(1) + state["fmydspWave0_idx"]) % jnp.int32(4)) 
		return state, jnp.stack([_result0,_result1,_result2,_result3,_result4,_result5,_result6,_result7,_result8,_result9,_result10,_result11,_result12,_result13]) 
