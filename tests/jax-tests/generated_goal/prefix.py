...
class mydsp(nn.Module):
	...
	def _initialize_carry(self, x: jnp.ndarray, length: int):
		...
		state["pfPerm0"] = np.float32(0.5)
		state["pfPerm1"] = np.float32(0.31)
		state["pfPerm2"] = np.float32(0.7)
		state["pfPerm3"] = np.float32(0.32)
		state["pfPerm4"] = np.float32(0.35)
	...
	def tick(self, params: dict, state: dict, inputs: jnp.array) -> Tuple[dict, jnp.ndarray]:
		...
		state["fVec0"] = jnp.float32(0.8) 
		state["pfPerm0"] = jnp.float32(0.8) 
		_result0 = pfTemp0 
		pfTemp1 = state["pfPerm1"] 
		state["fVec1"] = inputs[0] 
		state["pfPerm1"] = inputs[0] 
		_result1 = pfTemp1 
		pfTemp2 = state["pfPerm2"] 
		pfTemp3 = state["pfPerm3"] 
		state["fVec2"] = inputs[1] 
		state["pfPerm3"] = inputs[1] 
		state["pfPerm2"] = pfTemp3 
		_result2 = pfTemp2 
		pfTemp4 = state["pfPerm4"] 
		state["fVec3"] = jnp.float32(0.9) 
		state["pfPerm4"] = jnp.float32(0.9) 
		_result3 = pfTemp4 
		_result4 = pfTemp4 
		return state, jnp.stack([_result0,_result1,_result2,_result3,_result4]) 
