...
class mydsp(nn.Module):
	...
	def setup(self):
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("karplus") 
		ui_path.append("excitator") 
		self.add_hslider("fHslider0", ui_path, "excitation", 327.023, 2.0, 512.0, unnorm_funcs, "linear") 
		...
		ui_path.append("resonator") 
		self.add_hslider("fHslider2", ui_path, "attenuation", 0.0, 0.0, 1.0, unnorm_funcs, "linear") 
		self.add_hslider("fHslider3", ui_path, "duration", 271.994, 2.0, 512.0, unnorm_funcs, "linear") 
		ui_path.pop()
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow0 = (jnp.float32(1.0) / params["fHslider0"]) 
		fSlow1 = params["fButton0"] 
		fSlow2 = (jnp.float32(4.656613e-10) * params["fHslider1"]) 
		fSlow3 = (jnp.float32(0.5) * (jnp.float32(1.0) - params["fHslider2"])) 
		iSlow4 = (jnp.int32((params["fHslider3"] + jnp.float32(-1.5))) & jnp.int32(4095)).astype(jnp.int32) 
		...
		state["fVec0"] = fSlow1 
		state["fRec2"] = ((fRec2_temp + (((fSlow1 - fVec0_temp) > jnp.float32(0.0)).astype(jnp.int32))) - (fSlow0 * ((fRec2_temp > jnp.float32(0.0)).astype(jnp.int32)))) 
		state["fVec1"] = state["fVec1"].at[(state["IOTA0"] & 511).astype(jnp.int32)].set(((fSlow3 * (state["fRec0"][1] + state["fRec0"][2])) + (fSlow2 * (((state["fRec2"] > jnp.float32(0.0)).astype(jnp.int32)) * (state["iRec1"]))))) 
		state["fRec0"] = state["fRec0"].at[0].set(state["fVec1"][((state["IOTA0"] - iSlow4) & 511).astype(jnp.int32)]) 
		_result0 = state["fRec0"][0] 
