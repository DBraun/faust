...
class mydsp(nnx.Module):
	...
	def __init__(
		...
		self.add_hslider("fHslider2", ui_path, "excitation", 327.023, 2.0, 512.0, unnorm_funcs, "linear") 
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		fSlow1 = (jnp.float32(4.656613e-10) * params["fHslider1"]) 
		fSlow2 = params["fButton0"] 
		fSlow3 = (jnp.float32(1.0) / params["fHslider2"]) 
		...
		state["fVec0"] = fSlow2 
		state["fRec2"] = ((fRec2_temp + (((fSlow2 - fVec0_temp) > jnp.float32(0.0)).astype(jnp.int32))) - (fSlow3 * ((fRec2_temp > jnp.float32(0.0)).astype(jnp.int32)))) 
		state["fVec1"] = state["fVec1"].at[(state["IOTA0"] & 511).astype(jnp.int32)].set(((fSlow0 * (state["fRec0"][1] + state["fRec0"][2])) + (fSlow1 * ((state["iRec1"]) * ((state["fRec2"] > jnp.float32(0.0)).astype(jnp.int32)))))) 
