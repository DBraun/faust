...
class mydsp(nnx.Module):
	...
	def __init__(
		...
		self._imydspSIG0Wave0 = np.array([10,20,30,40,50], dtype=np.int32)
		...
		for i1 in range(0, 5):
			...
			imydspSIG0Wave0_idx = ((np.int32(1) + imydspSIG0Wave0_idx) % np.int32(5)) 
		...
		unnorm_funcs = {}
		ui_path = []
		ui_path.append("waveform5") 
		self.add_hslider("fHslider0", ui_path, "value", 0.0, 0.0, 1.0, unnorm_funcs, "linear") 
		ui_path.pop()
	...
	def tick(self, params: dict, state: dict, inputs: jnp.ndarray, rng: jax.Array = None) -> Tuple[dict, jnp.ndarray]:
		...
		iSlow0 = self._itbl0mydspSIG0[jnp.int32((jnp.float32(4.0) * params["fHslider0"]))] 
		fSlow1 = jnp.sin((iSlow0)) 
		_result0 = (inputs[0] * fSlow1) 
		_result1 = (iSlow0) 
