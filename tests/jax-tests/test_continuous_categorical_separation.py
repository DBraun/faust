...
if __name__ == "__main__":
    print("="*60)
    print("Testing Continuous/Categorical Separation for RL")
    print("="*60)

    all_passed = True

    for test_func in [
        test_continuous_only,
        test_with_nentry,
        test_rl_vmap_pattern,
        test_get_continuous_params_defaults_to_normalized,
    ]:
        try:
            if not test_func():
                all_passed = False
    ...
    if all_passed:
        ...
        print()
        print("RL Integration Pattern (Proper):")
        print()
        print("  # 1. Get continuous params from policy (Beta distribution)")
        print("  continuous = beta_policy(obs)  # [batch, num_continuous] in [0, 1]")
        print()
        print("  # 2. Get categorical params from policy (Categorical distribution)")
        print("  categorical_logits = categorical_policy(obs)  # [batch, num_options]")
        print()
        print("  # 3. Convert to dict format")
        print("  continuous_dict = model.get_continuous_params(batch_size, normalized=True)")
        print("  # Modify with policy outputs...")
        print()
        print("  # 4. Render with continuous params")
        print("  @jax.vmap")
        print("  def render(cont_params, inp, rng):")
        print("      return model(inp, normalized_params=cont_params, rngs=rng)")
        print()
        print("Continuous [0, 1] and categorical (logits) properly separated!")
    else:
